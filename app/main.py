...
async def websocket_endpoint(websocket: WebSocket, mode: str = "default"):
    ...
    try:
        ...
        system_instruction = (
            "Ты голосовой помощник. Не объясняй свои действия. "
            "Если нужно молчать — просто ничего не генерируй или выведи короткий токен. "
            "Если нужно говорить — говори сразу, без вступлений."
        )
                    ...
                    if isinstance(chunk, bytes):
                        ...
                        if should_send and resampled_audio:
                            ...
                            if len(audio_buffer) >= MIN_CHUNK_SIZE:
                                ...
                                logger.info(f"Sent {len(audio_buffer)} bytes of audio to client")
                                audio_buffer.clear()
